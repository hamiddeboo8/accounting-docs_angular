...
export function convertToEdit(doc: DocEdit): DocEditToSend {
    ...
    console.log(year)
    const docSend = new DocEditToSend(doc.ID, doc.DocNum, year, month, day, hour, minute, second, doc.AtfNum, doc.MinorNum, doc.Desc, doc.State, doc.DailyNum, doc.DocType, doc.EmitSystem, doc.AddDocItems, doc.RemoveDocItems)
    return docSend
}
...
export function convertFromEdit(docSend: DocEditToSend): DocEdit {
    var date = docSend.Year.toString().concat("-").concat(docSend.Month.toString()).concat('-').concat(docSend.Day.toString())
    date = date.concat(" ").concat(docSend.Hour.toString()).concat(":").concat(docSend.Minute.toString()).concat(":").concat(docSend.Second.toString())
    const doc = new DocEdit(docSend.ID, docSend.DocNum, date, docSend.AtfNum, docSend.MinorNum, docSend.Desc, docSend.State, docSend.DailyNum, docSend.DocType, docSend.EmitSystem, docSend.AddDocItems, docSend.RemoveDocItems)
    return doc
}